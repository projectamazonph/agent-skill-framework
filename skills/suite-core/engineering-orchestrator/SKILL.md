...
When the suite routes software work here, read [references/suite-contract.md](references/suite-contract.md). Preserve the Task Packet while coordinating the engineering specialists. Return the standard Handoff Packet only after quality-gates determines the engineering result.
...
| `solid-design` | Before adding or changing architectural boundaries; during design review; when test friction exposes coupling | Change map, boundary decision, contracts, dependency direction, and architecture risks |
| `test-driven-development` | Before changing production behavior or fixing a defect | Valid Red evidence, minimal Green change, and focused test result |
| `quality-gates` | After the requested behavior is complete and before reporting completion | PASS or BLOCKED with evidence and remediation route |
...
Invoke `solid-design` when any of these conditions apply:
...
Invoke `test-driven-development` before the first production-code change for every behavior addition or defect repair.
...
Invoke `quality-gates` only after all requested acceptance behavior appears complete.

Do not report completion unless the gate returns PASS. Route BLOCKED findings back to the appropriate phase:

| Finding | Return to |
| --- | --- |
| Missing or invalid behavior proof | `test-driven-development` at Red |
| Coupled or unjustified design | `solid-design` at Shape |
...
Use `solid-design` to define change axes, contracts, dependency direction, and test seams. Use the quality gate to check that the plan contains acceptance criteria and verification strategy. Do not pretend tests were run.
...
Reproduce first with `test-driven-development`. Keep the regression test. Use `solid-design` only if the root cause is structural or the fix adds a boundary.

### Behavior-Preserving Refactor

Characterize behavior first. Use `solid-design` to choose one structural move per loop. Do not mix unrelated feature work into the refactor.
...
Inspect through `solid-design`, then use `quality-gates` in review mode. Report findings first. Do not edit, send, publish, or open changes unless authorized.
