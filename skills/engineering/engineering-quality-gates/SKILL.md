...
Do not implement fixes. Return failures to `engineering-standards-orchestrator`, which routes remediation to the appropriate specialist.
...
- Missing or invalid Red/Green evidence → `tdd-workflow`
- Coupling, contract, responsibility, interface, or dependency issue → `solid-code-design`
