...
5. Return unexpected behavior changes to `tdd-workflow` rather than hiding them inside the refactor.
...
Do not claim whole-task completion. Return control to `engineering-standards-orchestrator`.
