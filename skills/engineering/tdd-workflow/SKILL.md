...
Do not own project sequencing, architecture policy, or final completion. Return evidence to `engineering-standards-orchestrator`, use `solid-code-design` for structural decisions, and let `loop-engineering` record and adjust the larger loop.
...
Improve names, local duplication, and cohesion only while the focused tests remain green. Invoke `solid-code-design` before extracting architectural boundaries, interfaces, strategies, adapters, or inheritance relationships.
